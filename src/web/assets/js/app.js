...
(function () {
  ...
  const StudyCardEngine = {
    init() {
      document.querySelectorAll('.study-card').forEach(card => {
        ...
        const restoreProgress = () => {
          if (!cardId) return;
          try {
            const saved = JSON.parse(localStorage.getItem(`hdh_card_${cardId}`) || 'null');
            if (saved && typeof saved.remembered === 'boolean') this.setProgress(card, rememberBtn, forgotBtn, saved.remembered);
          } catch (_) { /* Invalid local state is safely ignored. */ }
        };
        restoreProgress();
        ...
        if (rememberBtn) {
          rememberBtn.addEventListener('click', () => {
            ...
            this.setProgress(card, rememberBtn, forgotBtn, true);
          });
        }
        ...
        if (forgotBtn) {
          forgotBtn.addEventListener('click', () => {
            ...
            this.setProgress(card, rememberBtn, forgotBtn, false);
          });
        }
      });
    },
    ...
    setProgress(card, rememberBtn, forgotBtn, remembered) {
      card.dataset.state = remembered ? 'remembered' : 'forgotten';
      card.setAttribute('aria-label', remembered ? 'Thẻ đã thuộc' : 'Thẻ chưa nhớ');
      if (rememberBtn) rememberBtn.setAttribute('aria-pressed', String(remembered));
      if (forgotBtn) forgotBtn.setAttribute('aria-pressed', String(!remembered));
    }
  };
  ...
  const SubjectivePracticeEngine = {
    init() {
      document.querySelectorAll('.subjective-practice').forEach(container => {
        ...
        const stateKey = `hdh_practice_${practiceId}`;
        const saveState = () => {
        ...
        if (practiceId && textarea) {
          let saved = null;
          try { saved = JSON.parse(localStorage.getItem(stateKey) || 'null'); } catch (_) { saved = null; }
          const savedDraft = saved ? saved.draft : localStorage.getItem(`hdh_draft_${practiceId}`);
          if (savedDraft) textarea.value = savedDraft;
          if (saved) {
            checkboxes.forEach((cb, index) => { cb.checked = Boolean(saved.checked && saved.checked[index]); });
        ...
        if (compareBtn && rubricContainer) {
          compareBtn.addEventListener('click', () => {
            ...
            if (rubricContainer.classList.contains('visible')) {
              compareBtn.textContent = 'Ẩn Rubric tự kiểm tra';
            } else {
              compareBtn.textContent = 'So sánh với Rubric tự kiểm tra';
            }
            saveState();
          });
        }
      ...
      if (searchInput && resultsList) {
        searchInput.addEventListener('input', (e) => {
          ...
          const filtered = this.searchIndex.filter(item =>
            [item.title, item.summary, item.snippet, ...(item.headings || []), item.searchable_text]
